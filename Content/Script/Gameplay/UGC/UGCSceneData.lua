...
-- 运行时 spawn 钩子：所有经 WorldProjection 创建/接管的 Actor 都会触发一次。
-- AnimAgent / UGC runtime 资产靠它在 spawn 后注入 mesh，因此 Save/Load/Undo/Redo 全链路通用。
local _onActorCreated = nil

-- 具名 batch：由 begin_batch / end_batch 显式管理，跨多次原子调用共享同一个 group。
...
--- 注册 “actor spawn 后” 回调；fn 传 nil 表示注销。
--- @param fn function(actor, prefabName) | nil
...
function SceneData:Init(editorBridge)
    ...
    _projection = WorldProjection.New(editorBridge, _fireSpawnHook)
...
function SceneData:Clear()
    ...
    if _worldRuleAdapter and _worldRuleAdapter.reset then _worldRuleAdapter.reset() end
    _document = Document.New()
    _projection = WorldProjection.New(_bridge, _fireSpawnHook)
...
--- 开启（或复用）一个具名 batch，供 begin_batch / end_batch 工具与生成器共享。
--- 同名重复 begin 会复用已有 group，因此多次生成器调用可以归到同一个 batch。
--- @param name string 具名 batch 名（如 city_block）
...
function SceneData:BeginNamedBatch(name)
    ...
    _document:CreateGroup(id)
    _document:Touch()
    _activeBatch = id
    Log.Info("batch_opened", { batch = id, scope = "named" })
    return id
end

--- 关闭当前具名 batch，返回被关闭的 batchID；没有 active batch 时返回 nil。
function SceneData:EndActiveBatch()
    local id = _activeBatch
    _activeBatch = nil
    if id then Log.Info("batch_closed", { batch = id, scope = "named" }) end
    return id
end

--- 当前具名 batch（未开启时返回 nil）。生成器用它决定是复用还是新建 batch。
...
function SceneData:DeserializePackageTable(package)
    ...
    local stagedProjection = WorldProjection.New(_bridge, _fireSpawnHook)
